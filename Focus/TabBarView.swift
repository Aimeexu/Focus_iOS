    ...
    private func tabButton(imageName: String, tab: CustomTabBarView.Tab) -> some View {
        Button {
            ...
            Image(systemName: imageName)
                ...
                .offset(y: selectedTab == tab ? 15 : 0) // 选中时向下偏移到凹陷中
...
struct WaveTabBarBackground: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let dipDepth: CGFloat = 70  // 凹陷的深度
        let flatBottomWidth: CGFloat = 30  // 底部平滑区域的宽度
        ...
        path.addCurve(
            ...
            control1: CGPoint(x: dipStartX + 10, y: 0),
            control2: CGPoint(x: flatStartX - 8, y: dipDepth)
        )
        
        // 底部的平滑圆弧
        path.addCurve(
            to: CGPoint(x: flatEndX, y: dipDepth),
            control1: CGPoint(x: selectedPosition - 8, y: dipDepth + 3),
            control2: CGPoint(x: selectedPosition + 8, y: dipDepth + 3)
        )
        
        // 凹陷的右侧曲线 - 从平滑底部向上回到平面
        path.addCurve(
            to: CGPoint(x: dipEndX, y: 0),
            control1: CGPoint(x: flatEndX + 8, y: dipDepth),
            control2: CGPoint(x: dipEndX - 10, y: 0)
        )
