...
class UserManager: ObservableObject {
    ...
    // MARK: - 保存Facebook登录信息
    func saveFacebookLoginData(_ facebookLoginData: AchievementLoginData, loginMethod: String = "facebook") {
        let userDefaults = UserDefaults.standard
        
        // 保存认证信息
        userDefaults.set(facebookLoginData.accessToken, forKey: Keys.authToken)
        userDefaults.set(facebookLoginData.refreshToken, forKey: Keys.refreshToken)
        userDefaults.set(facebookLoginData.accessTokenName, forKey: Keys.accessTokenName)
        ...
        // 保存用户基本信息
        let user = facebookLoginData.user
        userDefaults.set(user.uuid, forKey: Keys.userUUID)
        userDefaults.set(user.account, forKey: Keys.userAccount)
        userDefaults.set(user.nickname ?? user.account, forKey: Keys.userNickname)
        userDefaults.set(user.phone, forKey: Keys.userPhone)
        ...
        // 转换Facebook用户物品为标准格式并保存
        let standardUserStuffs = user.userStuffs.allUserStuffs.map { facebookStuff in
            UserStuff(
                amount: facebookStuff.amount,
                createTime: facebookStuff.createTime,
                updateTime: facebookStuff.updateTime
            )
        }
        ...
        print("✅ Facebook登录信息已保存到 UserDefaults")
        printSavedUserInfo()
    }
...
// MARK: - FacebookSignInService 扩展，集成 UserManager
extension FacebookSignInService {
    func saveFacebookAuthData(_ facebookLoginData: AchievementLoginData) {
